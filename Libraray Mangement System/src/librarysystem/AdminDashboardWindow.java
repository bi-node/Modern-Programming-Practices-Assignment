...
public class AdminDashboardWindow extends JFrame implements ActionListener {
	...
	AdminDashboardWindow(String role) {
		...
		JLabel lblUser = new JLabel(role + " User");
		lblUser.setFont(new Font("SAN SERIF", Font.BOLD, 8));
		lblUser.setBounds(600, 55, 100, 15);
		p1.add(lblUser);
	...
	public void displayUser() {
		...
		for (Map.Entry<String, User> mapentry : userMap.entrySet()) {
	        ...
	        dashRecordTableModel.fireTableDataChanged();
	        p2.revalidate();
	        p2.repaint();
	...
	public void displayMember() {
		...
		for (Map.Entry<String, LibraryMember> mapentry : memberMap.entrySet()) {
			String[] rowData = { mapentry.getValue().getMemberId(), mapentry.getValue().getFirstName(),
					mapentry.getValue().getLastName(), mapentry.getValue().getTelephone() };
			dashRecordTableModel.addRow(rowData);
	        dashRecordTableModel.fireTableDataChanged();
	        p2.revalidate();
	        p2.repaint();
	...
	public void displayBooks() {
		...
		for (Map.Entry<String, Book> mapentry : memberMap.entrySet()) {
			String[] rowData = { mapentry.getValue().getIsbn(), mapentry.getValue().getTitle(),
					"" + mapentry.getValue().getCopyNums(), "" + mapentry.getValue().getMaxCheckoutLength() };
			dashRecordTableModel.addRow(rowData);
	        dashRecordTableModel.fireTableDataChanged();
	        p2.revalidate();
	        p2.repaint();
